...
thing in the state directory. Place names already resolved are unaffected;
only a future `init --geonames`/`--landmarks` has to download again.

While you are still changing which landmarks you collect, keep it:

```sh
harelphotos gc --keep-downloads
```
...
### AVIF, and what a browser that cannot read it gets

AVIF is the only format generated, because it is markedly smaller than WebP at
matched quality — measured on these photographs at equal or better SSIM, WebP
came out around half again as large, and JPEG several times larger. On a slow
uplink that is the whole difference.

Every current browser reads it. For anything old enough not to, one image URL
can return three formats: the server looks at the request's `Accept` header and
falls back to WebP, then JPEG. Those copies are **made on demand and cached**
rather than pre-generated — a parallel tree of every format would double the
disk to serve almost nobody — so the first such request is slow and every one
after it is a static file. They land beside the AVIF tree:

```
derived/256/2019/summer/IMG_1234.jpg.avif      generated by the scan
derived/webp/256/2019/summer/IMG_1234.jpg.webp made on demand, then cached
```

Because one URL can answer with three formats, those responses carry
`Vary: Accept`. That header is not optional: without it a cache can hand a
browser a format it cannot read, and since image URLs are immutable for a year,
that failure would be both silent and very long-lived.

Under `[encode]`, **`fallback = "none"`** turns the negotiation off and serves
AVIF to everyone. Worth it only if you know every device that will ever visit;
it saves nothing on disk, since the fallbacks are only made when asked for.

The rest of `[encode]` is best left alone. `format` is the generated format;
`speed` (0–10, default 6) trades encoding time against file size; `subsampling`
and `quality` control the encoder. All of them are part of the recipe
fingerprint, so changing any one re-encodes the entire collection — which on a
weak machine is measured in days. `[sizes]` is the exception worth knowing:
adding a size to `thumb` or `view` is deliberately *not* part of the
fingerprint, so it costs one extra encode per photograph and re-encodes
nothing.
...
in `config.toml`; the login page shows a Google button only when it is on.
Registering the OAuth client with Google is a handful of manual steps in their
console — INSTALL.md walks through them, including the one that catches
everybody, which is that an unpublished consent screen only works for accounts
you have listed as test users.

The division of labour is the part worth understanding: **Google asserts who
somebody is, and `users.toml` decides whether they may in.** A successful Google
sign-in for an address that is not in `users.toml` is refused. So enabling it
does not open the album to anyone with a Google account — it only saves the
people you have already invited from having a password here.

An account can have both a password and a `google` address, either alone.
A Google-only account simply omits `password`.
...
Three smaller `[ui]` settings, all about what a page shows:

- **`tagline`** is the line under the heading on the login page. It defaults to
  "By invitation only. Please login to continue."
- **`map_link`** decides where the "view on map" link in a photo's info panel
  goes: `"osm"` (the default, OpenStreetMap) or `"google"`. `"none"` drops the
  link and keeps the place name; `show_gps = false` removes the location line
  from the info panel and the album header entirely.
- **`dir_card_aspect`** is the shape of the cover picture on a subdirectory's
  card, `"4/3"` by default. Covers are cropped to fill it; `"native"` keeps each
  cover's own aspect ratio instead, which never crops but gives a ragged row of
  cards.
...
`journalctl -u harelphotos` works with no configuration at all. Run from a
terminal, it appears in the terminal. There is no log-file setting; redirect
the output if you want a file.

The commands log to stderr at warning level. `-v` adds informational lines,
`-vv` debug, `-q` errors only. Progress lines are separate from logging and go
to stdout, so `harelphotos scan >/dev/null` still shows you the problems and
none of the chatter.

Four things are always logged, because each is otherwise invisible: a
`.album.toml` that failed to parse, a photo whose images could not be
generated, a failed sign-in together with the address it came from, and any
request that returned a 500.

**Photo paths on successful requests are deliberately not logged.** A record of
who looked at which family photographs, and when, is a privacy liability that
serves no purpose here, and it is exactly the kind of file that accumulates
quietly for years and is then copied into a backup, a support ticket or a new
machine. Apache keeps its own access log if you want request-level records;
this application does not duplicate it.

## What requires a login

Everything, except a short list written out literally in one place
(`auth.PUBLIC_ENDPOINTS`): the landing page, logging in and out, the two Google
sign-in routes, `/static`, `/public`, the web manifest, `/privacy`, `/terms`,
and the health check. Every one of those serves fixed content and touches no
photo data. Signing in cannot require being signed in; clearing a session needs
no session; the manifest is fetched before anyone has signed in; and `/privacy`
and `/terms` exist only because Google's consent screen demands publicly
fetchable URLs.

The gate is a single hook that runs before every request, so the default is
"needs a session" and being public is the thing you have to ask for. The
opposite arrangement — public by default, restricted per route — is a footgun:
a route added later would be exposed by omission rather than protected by it.
That exemption list is the highest-risk few lines in the application, so a test
